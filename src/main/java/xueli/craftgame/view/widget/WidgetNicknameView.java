...
public class WidgetNicknameView extends ViewWidget {
	...
	public void draw(long nvg) {
		...
		float textWidth = nvgText(nvg, 0, -114514, CraftGame.INSTANCE_CRAFT_GAME.getPlayerStat().getName());

		// 然后画背景
		nvgBeginPath(nvg);
		nvgRect(nvg, x.getValue() - textWidth - height.getValue() / 2 * icon_scale - icon_name_space.getValue(),
				y.getValue() - icon_scale_width.getValue(), textWidth + height.getValue() / 2 * icon_scale
						+ icon_name_space.getValue() + icon_scale_width.getValue(),
				height.getValue() + icon_scale_width.getValue() * 2);
		...
		nvgText(nvg, x.getValue(), y.getValue(), CraftGame.INSTANCE_CRAFT_GAME.getPlayerStat().getName());

		// 头像
		nvgImagePattern(nvg, x.getValue() - textWidth - height.getValue() * icon_scale - icon_name_space.getValue(),
				y.getValue() - (icon_scale - 1.0f) * height.getValue() / 2, height.getValue() * icon_scale,
				height.getValue() * icon_scale, 0, CraftGame.INSTANCE_CRAFT_GAME.getPlayerStat().getIconTexture(), 1,
				paint);
		nvgBeginPath(nvg);
		nvgCircle(nvg, x.getValue() - textWidth - height.getValue() / 2 * icon_scale - icon_name_space.getValue(),
				y.getValue() + height.getValue() / 2, height.getValue() / 2 * icon_scale);
