...
public class StateServerSelect extends NVGRenderer {
	...
	private static EvalableFloat list_width = new EvalableFloat(
			list_border_width.getExpression() + " - 2 * (" + list_border.getExpression() + ")");
	private static EvalableFloat list_border_x = new EvalableFloat("win_width * 0.25");
	private static EvalableFloat list_x = new EvalableFloat(
			list_border_x.getExpression() + " + (" + list_border.getExpression() + ")");
	private static EvalableFloat list_height = new EvalableFloat(
			border_height.getExpression() + " - 2 * (" + list_border.getExpression() + ")");
	private static EvalableFloat list_y = new EvalableFloat(
			border_y.getExpression() + " + " + list_border.getExpression());
	...
	public void stroke() {
		...
		{
			nvgBeginPath(nvg);
			nvgRect(nvg, list_border_x.getValue(), border_y.getValue(), list_border_width.getValue(),
					border_height.getValue());
			// nvgRect(nvg, list_x.getValue(), list_y.getValue(), list_width.getValue(),
			// list_height.getValue());
	...
	private void readServerList() {
		...
		try {
			JsonObject object = gson.fromJson(
					new FileReader(new File(CraftGame.DEFAULT_WORKING_DIRECTORY_STRING + "/serverlist.json")),
					JsonObject.class);
			...
			e.printStackTrace();
			game.getRendererManager().message("Read Server List Failed!",
					"Please check console: " + e.getClass().getName(), Type.ERROR);
			return;
		}
	...
	private void saveServerList() {
		...
		try {
			...
			e.printStackTrace();
			game.getRendererManager().message("Save Server List Failed!",
					"Please check console: " + e.getClass().getName(), Type.ERROR);
			return;
		}
