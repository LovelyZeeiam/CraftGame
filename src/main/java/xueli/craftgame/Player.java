...
public class Player {
	...
	public void tick() {
		if(display.isMouseGrabbed()) {
			...
			pos.rotY += display.getCurson_dx() * 0.08f;
			...
			if(display.isMouseDown(GLFW.GLFW_MOUSE_BUTTON_LEFT)) {
				...
				if(lastSelectedBlock != null && Time.thisTime - lastTimeOperationBlock > 100) {
					dimension.setBlock(lastSelectedBlock.getX(), lastSelectedBlock.getY(), lastSelectedBlock.getZ(), new Tile(StateWorld.getInstance().getBlocks().getBlockBase("craftgame:stone")));
					lastTimeOperationBlock = Time.thisTime;
				}
