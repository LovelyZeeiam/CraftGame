...
public class EvalableFloat {
	...
	private void eval() {
		...
		// Because of too many references of this expression, I choose not to change it to a new format: ${variable_name}
		...
		varValues.forEach((k,v) -> newExpressionString.replaceAll(MessageFormat.format("${{{0}}}",k), "(" + v.toString() + ")"));
