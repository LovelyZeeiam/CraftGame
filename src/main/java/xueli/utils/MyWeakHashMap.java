 ...
 * questions.
 * 
 * Copied from JDK source but add external ReferenceQueue support!
 * 
 */
...
/**
 * Hash table based implementation of the {@code Map} interface, with <em>weak
 * keys</em>. An entry in a {@code WeakHashMap} will automatically be removed
 ...
 * characteristics similar to those of the {@code HashMap} class, and has the
 ...
 * {@code WeakHashMap} may be constructed using the
 ...
 * {@code equals} methods test for object identity using the {@code ==}
 * operator. Once such a key is discarded it can never be recreated, so it is
 * impossible to do a lookup of that key in a {@code WeakHashMap} at some later
 * time and be surprised that its entry has been removed. This class will work
 * perfectly well with key objects whose {@code equals} methods are not based
 * upon object identity, such as {@code String} instances. With such recreatable
 * key objects, however, the automatic removal of {@code WeakHashMap} entries
 * whose keys have been discarded may prove to be confusing.
 *
 * <p>
 * The behavior of the {@code WeakHashMap} class depends in part upon the
 * actions of the garbage collector, so several familiar (though not required)
 * {@code Map} invariants do not hold for this class. Because the garbage
 * collector may discard keys at any time, a {@code WeakHashMap} may behave as
 * though an unknown thread is silently removing entries. In particular, even if
 * you synchronize on a {@code WeakHashMap} instance and invoke none of its
 * mutator methods, it is possible for the {@code size} method to return smaller
 * values over time, for the {@code isEmpty} method to return {@code false} and
 * then {@code true}, for the {@code containsKey} method to return {@code true}
 * and later {@code false} for a given key, for the {@code get} method to return
 * a value for a given key but later return {@code null}, for the {@code put}
 * method to return {@code null} and the {@code remove} method to return
 * {@code false} for a key that previously appeared to be in the map, and for
 * successive examinations of the key set, the value collection, and the entry
 * set to yield successively smaller numbers of elements.
 *
 * <p>
 * Each key object in a {@code WeakHashMap} is stored indirectly as the referent
 * of a weak reference. Therefore a key will automatically be removed only after
 * the weak references to it, both inside and outside of the map, have been
 * cleared by the garbage collector.
 *
 * <p>
 * <strong>Implementation note:</strong> The value objects in a
 * {@code WeakHashMap} are held by ordinary strong references. Thus care should
 ...
 * the {@code WeakHashMap} itself; that is, a value object may strongly refer to
 * some other key object whose associated value object, in turn, strongly refers
 * to the key of the first value object. If the values in the map do not rely on
 * the map holding strong references to them, one way to deal with this is to
 * wrap values themselves within {@code WeakReferences} before inserting, as in:
 * {@code m.put(key, new WeakReference(value))}, and then unwrapping upon each
 * {@code get}.
 *
 * <p>
 ...
 */
public class MyWeakHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
	...
	private final Consumer<V> keyRemovedConsumer;

	/**
	 * The number of times this WeakHashMap has been structurally modified.
	 * Structural modifications are those that change the number of mappings in the
	 * map or otherwise modify its internal structure (e.g., rehash). This field is
	...
	/**
	 * Constructs a new, empty {@code WeakHashMap} with the given initial capacity
	 ...
	 */
	public MyWeakHashMap(int initialCapacity, float loadFactor, ReferenceQueue<Object> rq,
			Consumer<V> keyRemovedConsumer) {
		...
		this.queue = rq;
		this.keyRemovedConsumer = keyRemovedConsumer;
	 ...
	 */
	public MyWeakHashMap(int initialCapacity, ReferenceQueue<Object> rq, Consumer<V> keyRemovedConsumer) {
		this(initialCapacity, DEFAULT_LOAD_FACTOR, rq, keyRemovedConsumer);
	}

	/**
	 * Constructs a new, empty {@code WeakHashMap} with the default initial capacity
	 * (16) and load factor (0.75).
	 */
	public MyWeakHashMap(ReferenceQueue<Object> rq, Consumer<V> keyRemovedConsumer) {
		this(DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, rq, keyRemovedConsumer);
	}

	/**
	 * Constructs a new {@code WeakHashMap} with the same mappings as the specified
	 * map. The {@code WeakHashMap} is created with the default load factor (0.75)
	 ...
	 * @since 1.3
	 */
	public MyWeakHashMap(Map<? extends K, ? extends V> m, ReferenceQueue<Object> rq, Consumer<V> keyRemovedConsumer) {
		this(Math.max((int) ((float) m.size() / DEFAULT_LOAD_FACTOR + 1.0F), DEFAULT_INITIAL_CAPACITY),
				DEFAULT_LOAD_FACTOR, rq, keyRemovedConsumer);
		putAll(m);
	}
			...
			synchronized (queue) {
				...
				while (p != null) {
					...
					if (p == e) {
						...
						keyRemovedConsumer.accept(e.value);
	...
	static class WeakHashMapSpliterator<K, V> {
		...
		MyWeakHashMap.Entry<K, V> current; // current node
		int index; // current index, modified on advance/split
		int fence; // -1 until first use; then one past last index
		int est; // size estimate
		int expectedModCount; // for comodification checks

		WeakHashMapSpliterator(MyWeakHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			this.map = m;
			this.index = origin;
			this.fence = fence;
			this.est = est;
	...
	static final class KeySpliterator<K, V> extends WeakHashMapSpliterator<K, V> implements Spliterator<K> {
		KeySpliterator(MyWeakHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			...
			if (tab.length >= hi && (i = index) >= 0 && (i < (index = hi) || current != null)) {
				...
				do {
					if (p == null)
						...
						if (x != null) {
							@SuppressWarnings("unchecked")
							K k = (K) MyWeakHashMap.unmaskNull(x);
							action.accept(k);
						}
					}
				} while (p != null || i < hi);
	...
	static final class ValueSpliterator<K, V> extends WeakHashMapSpliterator<K, V> implements Spliterator<V> {
		ValueSpliterator(MyWeakHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
	...
	static final class EntrySpliterator<K, V> extends WeakHashMapSpliterator<K, V>
		...
		EntrySpliterator(MyWeakHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
