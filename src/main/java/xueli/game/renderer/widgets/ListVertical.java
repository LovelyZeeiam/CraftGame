...
public class ListVertical extends IWidget {
	...
	private ArrayList<Boundary> entries_boundary = new ArrayList<>();
	private ArrayList<IListEntry> entries = new ArrayList<>();

	public ListVertical(EvalableFloat x, EvalableFloat y, EvalableFloat width, EvalableFloat entry_height,
			EvalableFloat visibleAreaHeight, ArrayList<IListEntry> entries) {
	...
	private Boundary calculateBoundary(int element_count) {
		EvalableFloat entry_y = new EvalableFloat(MessageFormat.format("({0}) + ({1}) * (({2}) + {3})",
				y.getExpression(), element_count, entry_height.getExpression(), margin.getExpression()));
		return new Boundary(x, entry_y, entry_width, entry_height);
	}

	private void calculateMaxHeight() {
		this.max_height = new EvalableFloat(MessageFormat.format("({0}) * ({1}) + ({0} - 1) * ({2})", entries.size(),
				entry_height.getExpression(), margin.getExpression()));
	...
	public void stroke(long nvg, String fontName) {
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			Boundary boundary = entries_boundary.get(i);
			...
			entry.stroke(nvg, i, boundary.getWidth().getValue(), boundary.getHeight().getValue(), fontName, isHover,
					isClicked);
		...
		// wheel
		{
			...
			float wheel_y_offset = (height.getValue() - wheel_length)
					* (wheel_offset / (max_height.getValue() - height.getValue()));

			nvgBeginPath(nvg);
			nvgRect(nvg, x.getValue() + entry_width.getValue() + 3.0f * Game.INSTANCE_GAME.getDisplayScale(),
					y.getValue() + wheel_y_offset, 4.0f * Game.INSTANCE_GAME.getDisplayScale(), wheel_length);
	...
	public void update() {
		...
		if (this.max_height.getValue() <= height.getValue())
			this.wheel_offset = 0;
		else if (this.wheel_offset < 0)
			this.wheel_offset = 0;
		...
		if (isMouseHover() && !(isMouseInWheel || isDragWheel)) {
			...
			if (Game.INSTANCE_GAME.getDisplay().isMouseDownOnce(GLFW.GLFW_MOUSE_BUTTON_LEFT)
					&& (hover_entry_id >= 0 && hover_entry_id < entries.size())) {
	...
	public void size() {
		...
		for (Boundary boundary : entries_boundary) {
			boundary.size();
		}
