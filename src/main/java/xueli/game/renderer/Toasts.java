...
public class Toasts extends NVGRenderer {
	...
	private static final NVGColor fine_title_color = NVGColor.create(), exception_title_color = NVGColor.create(),
			error_title_color = NVGColor.create(), confusion_title_color = NVGColor.create();
	private static final NVGColor fine_content_color = NVGColor.create(), exception_content_color = NVGColor.create(),
			error_content_color = NVGColor.create(), confusion_content_color = NVGColor.create();
	...
	public Toasts() {
		...
		this.tex_toast = nvgCreateImage(nvg, CraftGame.DEFAULT_RES_DIRECTORY_STRING + "/gui/toast/toast_1.png",
				NVG_IMAGE_NEAREST | NVG_IMAGE_GENERATE_MIPMAPS);

		this.tex_error = nvgCreateImage(nvg, CraftGame.DEFAULT_RES_DIRECTORY_STRING + "/gui/toast/toast_error.png",
				NVG_IMAGE_GENERATE_MIPMAPS);
		this.tex_fine = nvgCreateImage(nvg, CraftGame.DEFAULT_RES_DIRECTORY_STRING + "/gui/toast/toast_fine.png",
				NVG_IMAGE_GENERATE_MIPMAPS);
		this.tex_confusion = nvgCreateImage(nvg,
				CraftGame.DEFAULT_RES_DIRECTORY_STRING + "/gui/toast/toast_confusion.png", NVG_IMAGE_GENERATE_MIPMAPS);
		this.tex_exception = nvgCreateImage(nvg,
				CraftGame.DEFAULT_RES_DIRECTORY_STRING + "/gui/toast/toast_exception.png", NVG_IMAGE_GENERATE_MIPMAPS);
	...
	public void stroke() {
		...
		for (Toast toast : toasts) {
			...
			if (aliveTime < TOAST_FADE_IN_AND_OUT_TIME) {
				...
				// fade out
				fade_progress = (float) Math
						.cos(((aliveTime - TOAST_FADE_IN_AND_OUT_TIME - toast.aliveTime) / TOAST_FADE_IN_AND_OUT_TIME)
								* (Math.PI / 2));
			} else {
				fade_progress = 1.0f;
			}

			nvgTranslate(nvg, game.getWidth() - (width + offset_x) * game.getDisplayScale() * fade_progress,
					offset_y * game.getDisplayScale() + (margin + height) * game.getDisplayScale() * toast.index);
			...
			// background
			{
				nvgImagePattern(nvg, 0, 0, width * game.getDisplayScale(), height * game.getDisplayScale(), 0,
						tex_toast, 1, paint);
				nvgBeginPath(nvg);
			...
			// icon
			{
				...
				nvgImagePattern(nvg, icon_margin, icon_margin, real_size, real_size, 0, getIconTexture(toast.type), 1,
						paint);
				nvgBeginPath(nvg);
			...
			// title
			{
				...
				nvgText(nvg, (icon_margin + icon_size + icon_push_offset) * game.getDisplayScale(),
						content_margin * game.getDisplayScale(), toast.title);
			...
			// message
			{
				...
				nvgTextBox(nvg, (icon_margin + icon_size + icon_push_offset) * game.getDisplayScale(),
						height * game.getDisplayScale() / 5 * 3, width * game.getDisplayScale() - 2 * icon_margin
								- icon_size * game.getDisplayScale() - 2 * content_margin,
						toast.message);
	...
	private void calculateScreen() {
		this.maxDisplay = (int) ((game.getHeight() - 2 * Toasts.offset_y * game.getDisplayScale())
				/ (Toasts.margin + Toasts.height) / game.getDisplayScale());
