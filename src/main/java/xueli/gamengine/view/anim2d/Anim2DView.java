...
public class Anim2DView extends View {
    ...
    private static Anim2D processAnim(JsonObject data) {
        ...
        author.forEach(e -> {
            ...
            Logger.checkNullAndThrow(descElement, getErrorMessage("element \"description\" in \"author\" can't be found!"));

            anim.authors.add(new Author(nameElement.getAsString(), descElement.getAsString()));

            Logger.info("Find author: name: " + nameElement.getAsString() + ", description: " + descElement.getAsString());

        });

        // scene
        JsonObject sceneObj = data.getAsJsonObject("scene");
        Logger.checkNullAndThrow(sceneObj, "element \"scene\" can't be found! But it is still good with a black screen~(neh heh heh)");
        ...
        sceneObj.entrySet().forEach(e -> {
            ...
            switch (type) {
                case "text":
                    ...
                    Logger.checkNullAndThrow(textElement, "element \"text\" in scene \"" + name + "\" can't be found: " + name);
                    String text = textElement.getAsString();

                    JsonElement widthElement = o.get("width");
                    Logger.checkNullAndThrow(widthElement, "element \"width\" in scene \"" + name + "\" can't be found: " + name);
                    String width = widthElement.getAsString();

                    JsonElement heightElement = o.get("height");
                    Logger.checkNullAndThrow(heightElement, "element \"height\" in scene \"" + name + "\" can't be found: " + name);
                    String height = heightElement.getAsString();

                    JsonElement sizeElement = o.get("size");
                    Logger.checkNullAndThrow(sizeElement, "element \"size\" in scene \"" + name + "\" can't be found: " + name);
                    ...
                    NVGColor color = colorElement == null ? GuiColor.BLACK : GuiResource.loadColor(colorElement.getAsJsonArray());

                    ElementTextView textView = new ElementTextView(name, color, new EvalableFloat(size), new EvalableFloat(width), new EvalableFloat(height), align, text);
        ...
        for (Class<?> clazz : ClassFinder.findClassByPackage("xueli.gamengine.view.anim2d")) {
            if (clazz.getSimpleName().equals("AnimComponent")) continue;
        ...
        Logger.checkNullAndThrow(animObj, "element \"anim\" can't be found! But it is still good with a static image~(neh heh heh)");
