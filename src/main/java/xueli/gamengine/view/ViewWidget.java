...
public abstract class ViewWidget implements AutoCloseable {
    ...
    protected void drawBorder(long nvg) {
        ...
        // 上边框
        nvgRoundedRect(nvg, x.getValue() - borderWidth, y.getValue() - borderWidth, borderWidth * 2 + width.getValue(), borderWidth, 0);
        // 左边框
        nvgRoundedRect(nvg, x.getValue() - borderWidth, y.getValue(), borderWidth, height.getValue(), 0);
        // 下边框
        nvgRoundedRect(nvg, x.getValue() - borderWidth, y.getValue() + height.getValue(), borderWidth * 2 + width.getValue(), borderWidth, 0);
        // 右边框
    ...
    protected boolean isMouseHover() {
        ...
        return cursorX > x.getValue() & cursorX < x.getValue() + width.getValue() & cursorY > y.getValue() & cursorY < y.getValue() + height.getValue();
    }
