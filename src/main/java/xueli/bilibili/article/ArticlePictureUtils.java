...
public class ArticlePictureUtils {
	...
	public static void saveAllPicture(int cvId, String saveFolder) {
		...
		ThreadPoolExecutor poolExecutor = new ThreadPoolExecutor(20,25,5L,
	...
	public static ArrayList<Integer> searchArticle(String key) {
		try {
			...
			{
				URL url = new URL("https://api.bilibili.com/x/web-interface/search/type?search_type=article&keyword=" + key);
				
				InputStream urlInputStream = url.openStream();
				JsonObject pageUrlJsonObject = new Gson().fromJson(new InputStreamReader(urlInputStream), JsonObject.class);
				urlInputStream.close();
				
				int code = pageUrlJsonObject.get("code").getAsInt();
				if(code != 0) {
					Log.logger.warning("Article Search Failed: " + pageUrlJsonObject.get("message").getAsString() + ", " + key);
					return null;
				}
			...
			for(int i = 1; i <= pageCount; i++) {
				URL url = new URL("https://api.bilibili.com/x/web-interface/search/type?search_type=article&page=" + i + "&keyword=" + key);
				
				InputStream urlInputStream = url.openStream();
				JsonObject pageJsonObject = new Gson().fromJson(new InputStreamReader(urlInputStream), JsonObject.class);
				urlInputStream.close();
				
				int code = pageJsonObject.get("code").getAsInt();
				if(code != 0) {
					Log.logger.warning("Article Search Failed: " + pageJsonObject.get("message").getAsString() + ", " + key);
					continue;
				}
