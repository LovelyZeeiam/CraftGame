...
//		System.out.println(CollectionHelper.toString(chunkMap));
		...
		for(int i = 0; i < Chunk.CHUNK_SIZE; i++) {
			for(int j = 0; j < Chunk.CHUNK_SIZE; j++) {
				chunk.setBlockImmediate(i, 70, j, chunkMap[i][j] == 0 ? GameRegistry.BLOCK_STONE : GameRegistry.BLOCK_DIRT);
			}
		}
...
//					System.out.println(x * LAYER_TO_CHUNK_COUNT + x1);
				...
				this.layerMap.put(new Vector2i((offsetX << CHUNK_POS_TO_LAYER_OFFSET) + x, (offsetZ << CHUNK_POS_TO_LAYER_OFFSET) + z), chunkMap);
