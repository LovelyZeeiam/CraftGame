...
public class PickCollider {
	...
	public static PickResult pick(Vector camera, double maxReachDistance, ListenableBufferedWorldAccessible world) {
		Vector3d direction = new Vector3d(TriFuncMap.sin(camera.rotY), TriFuncMap.tan(camera.rotX),
				-TriFuncMap.cos(camera.rotY));
		direction.normalize();

		// If the line goes down, it is impossible to interact with the bottom.
		// Conversely, the line going up can't interact with the top. The thesis is the
		// same with the other 2 axis.
		...
		for (double i = 0; i < maxReachDistance; i += 0.1) {
			...
			Vector3i blockPos = new Vector3i((int) Math.floor(rayEndX), (int) Math.floor(rayEndY),
					(int) Math.floor(rayEndZ));
				...
				for (int j = 0; j < blockBoundingBoxes.size(); j++) {
					...
					if (needFaceTest[0]) {
						...
						if (t >= 0 && t < finalT) {
							...
							if (thisY > b0.y && thisY < b1.y && thisZ > b0.z && thisZ < b1.z) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.X_MINUS,
										new Vector2d((thisY - b0.y) / (b1.y - b0.y), (thisZ - b0.z) / (b1.z - b0.z)));
							}
						}
					}
					...
					if (needFaceTest[1]) {
						...
						if (t >= 0 && t < finalT) {
							...
							if (thisY > b0.y && thisY < b1.y && thisZ > b0.z && thisZ < b1.z) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.X_PLUS,
										new Vector2d((thisY - b0.y) / (b1.y - b0.y), (thisZ - b0.z) / (b1.z - b0.z)));
							}
						}
					}
					...
					if (needFaceTest[2]) {
						...
						if (t >= 0 && t <= finalT) {
							...
							if (thisX > b0.x && thisX < b1.x && thisZ > b0.z && thisZ < b1.z) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.Y_MINUS,
										new Vector2d((thisX - b0.x) / (b1.x - b0.x), (thisZ - b0.z) / (b1.z - b0.z)));
							}
						}
					}
					...
					if (needFaceTest[3]) {
						...
						if (t >= 0 && t <= finalT) {
							...
							if (thisX > b0.x && thisX < b1.x && thisZ > b0.z && thisZ < b1.z) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.Y_PLUS,
										new Vector2d((thisX - b0.x) / (b1.x - b0.x), (thisZ - b0.z) / (b1.z - b0.z)));
							}
						}
					}
					...
					if (needFaceTest[4]) {
						...
						if (t >= 0 && t <= finalT) {
							...
							if (thisX > b0.x && thisX < b1.x && thisY > b0.y && thisY < b1.y) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.Z_MINUS,
										new Vector2d((thisX - b0.x) / (b1.x - b0.x), (thisY - b0.y) / (b1.y - b0.y)));
							}
						}
					}
					...
					if (needFaceTest[5]) {
						...
						if (t >= 0 && t <= finalT) {
							...
							if (thisX > b0.x && thisX < b1.x && thisY > b0.y && thisY < b1.y) {
								finalT = t;
								result = new PickResult(blockPos, aabb, BoxFace.Z_PLUS,
										new Vector2d((thisX - b0.x) / (b1.x - b0.x), (thisY - b0.y) / (b1.y - b0.y)));
							}
						}
					}
